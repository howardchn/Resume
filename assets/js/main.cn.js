var appDataCn = {
    title: '陈浩',
    description:
        '熟悉多种开发语言(.NET, Node.js, Python, Go, React, Dart)，全栈能力。具有7年技术产品经理（包含4年技术产品总监），8年多的项目组长，3年多技术总监的管理经验。',
    currentPosition: '技术产品总监',
    basicInfo: '男 ︳本科双学位 ︳ 20年工作经验 ︳现住成都',
    experiences: [
        {
            type: 'work',
            title: '工作经验',
            details: [
                {
                    ...
                    summary: '技术产品总监',
                    date: '2023/10 - Now',
                    work: 'Empathia 是一家专注于为医疗助手提供人工智能记录服务的初创公司。Empathia 和前一家公司由同一位创始人创建，并由前公司的一些核心成员转移过来。虽然我的title是director，实际负责产品家族中多个主要产品的设计，开发，协调以及测试发布，以及后续的迭代。并在自己负责的几个产品线上，通盘考虑其他模块的交互以及后续的发展方向。',
                },
                {
                    name: 'MobLab',
            ...
            details: [
                {
                    ...
                    summary: '双学士学位（绘画，设计为第一专业 + 软件工程为第二专业）',
                    date: '2000 - 2004',
            ...
            details: [
                {
                    ...
                    url: 'https://empathia.ai',
                    summary:
                        'Empathia AI 提供了一个强大的 Chrome 扩展程序，无缝集成到临床医生的工作流程中，使他们能够直接在其电子病历（EMR）系统中访问AI医疗助手。通过此扩展程序，临床医生可以高效地在其电子病历旁收录和病人的对话，支持多语言听写，消除下班后的文档书写和整理工作。Empathia AI 通过自动转译音频，生成笔记和提供可自定义的模板来增强患者护理，即使在离线状态也能做到基本的录音和备份功能，使临床医生能够将优先事项放在患者问诊上而不是文书工作上。',
                    date: '2023/10 - Now',
                    work: '负责所有流程的设计和开发。在这个项目中，我使用了许多由Extension APIs提供的新功能，例如Service Worker、messenger、side-panel、content script、offscreen等，以及一些比较酷的功能，如WebRTC、indexdb，以建立良好的体验，包括音频录制、离线模式、页面内网页扫描（包括PDF和图片）、静音检测和通知等。我还需要整理来自customer success团队的需求并无缝的将功能集成到项目中。该项目仍在迭代，目前每天为1500多名用户提供服务。',
                },
                {
                    name: 'Empathia Audio Server',
                    summary:
                        'Empathia Audio Server是一个强大的音频处理微服务，专为满足Empathia AI的所有音频转译相关需求而设计。它为音频备份、转录和分析提供了强大的后端支持，确保临床医生可以高效地管理他们的音频数据。该服务器基于Golang构建，是Empathia AI生态系统中的关键组件。',
                    date: '2024/06 - Now',
                    work: '我是这个项目的唯一开发人员。项目主要使用 Golang 和 OpenAI 的语音转文字服务，将音频存档为文本并安全存储音频文件。除此之外，音频还有各种问题会影响转录，如音量低、噪音、静默间隙、多语言和一些小语种、以及客户端上的数据丢失的恢复等。因此，解决这些问题是此服务的一个挑战。项目仍在迭代中。',
                },
                {
                    name: 'Empathia Knowledge Base System',
                    summary:
                        'Empathia 知识库系统是一个强大的知识管理系统，旨在支持 Empathia AI 产品家族中部分医生对病人历史病例需求。它允许用户上传大量文档，包括 PDF、txt、图像、word 文件，并将其转换为文本内容，将原始文本和矢量数据存储在系统中。然后可以基于上传的材料进行搜索和撰写新文档。它目前处于内部测试阶段，将在不久的将来发布。现在，内部测试人员主要是实际的医生，和极少数的律师。目前的反馈还不错。',
                    date: '2024/10 - Now',
                    work: '我是这个项目的唯一开发，设计以及产品经理。系统是基于 React 和 Node.js 构建的，并使用 MongoDB 和 Chroma 进行矢量存储。主要的挑战是要在大量上传文件上构建几乎实时的搜索和撰写新文档的系统。最近，OpenAI 升级了嵌入模型，这在之前没有考虑到。我仍在努力使其与新模型兼容。',
                },
                {
                    name: 'MobLab Kids',
                    summary:
                        'MobLab Kids 是一款应用程序，通过行为游戏评估和跟踪3至8岁儿童的各种技能指标，同时提供专家建议。这个应用程序还开发了一系列相关的SaaS服务和应用程序，如托儿日记、每日培训知识系统和幼儿园管理SaaS系统。',
                    date: '2020/06 - 2022/12',
                    work: '负责整个产品线的设计、架构、研发、提交、核心功能开发，以及后续的数据分析和迭代输出。这条产品线包括两个使用 Flutter 开发的移动应用程序（面向消费者/企业），一个使用 React 开发的Web应用，以及一个使用 Node.js 开发的后端服务（基于 SaaS 的多租户）。整个产品线基于微服务架构构建。后端服务部署在 AWS 上，并使用 MongoDB 作为主要数据库。同时，它使用 OpenAI 服务来支持许多功能，如教师/家长聊天、制定培训计划、生成儿童成长知识库等。',
                },
                {
                    name: 'Learning Behavior Analysis System',
                    summary:
                        '这个系统最初为GSX开发，用于分析在线课程中的学生行为。后来扩展为多租户模型，为作业帮和开放大学提供SaaS服务。它由行为数据录入、分析服务和前端展示平台组成。作业帮每天从其Hadoop数据库中提取上课出勤率、屏幕在线、眼动和课后活动等行为数据，并将其上传到阿里云OSS存储。我们定期抓取、增量更新和清理这些数据，然后将其存储在MySQL数据库中供数据科学团队使用。数据团队每周对这些数据进行训练，以生成模型，并每日生成每位学生的表现报告。GSX的教师使用该系统跟踪学生进步、制定学习计划，并通过提供建议和讨论话题来促进与家长的沟通。以优质的服务提高获客率和留存率。',
                    date: '2020/04 - 2020/08',
                    work: '负责整个产品线的用户体验设计、架构、研发、提交，并与GSX和作业帮进行沟通。同时负责数据拉取，并与数据科学团队合作进行接口设计和开发，还承担部分UI开发任务。',
                },
                {
                    name: '智能告警压减',
                ...
                },
                {
                    ...
                    summary:
                        '根据用户选定的一段时序数据进行分析，找到当前数据中的异常部分，并渲染出来，以图表形式展现给用户。同时在这个图表上还设计了一系列可用的辅助功能。',
                ...
                },
                {
                    name: 'StatsD 数据推送功能',
                    summary:
                        'StatsD 是一个开源项目，支持推送的方式把用户数据发送到一个指定的平台。而该项目的目的就是支持用户推送数据到LM平台里。',
                ...
                },
                {
                    ...
                    summary:
                        '当用户在LM平台删除一个资源，会被永久删除掉。而往往用户删除后可能会返回，或者出现错误删除等情况时，希望能快速恢复被删除的设备。减少人工干预和减少恢复的时间。',
                ...
                },
                {
                    ...
                    summary:
                        '多时区支持对国际化软件是一个重要的功能。该项目在LM平台上支持用户定义的多时区查看和计划事务设定。',
                ...
                },
                {
                    ...
                    summary:
                        '对现有windows collector进行签名，避免在用户安装时遇到不友好的Smart Screen。增强用户安装体验。',
